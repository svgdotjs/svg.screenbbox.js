...
SVG.extend(SVG.Polyline, SVG.Polygon, SVG.Path, {
  ...
  screenBBox: function(){
    ...
    var arr = this.array().valueOf()          // path/point array
    ...
    // reinitiate i cause we already used it
    i = arr.length
    while(i--){
      ...
      // only y
        ...
        pos[0] = arr[i][1]
        arr[i][1] = transformPoints([pos.slice()])[0]
      // only x
      }else if (s == 'V'){
        pos[1] = arr[i][1]
        arr[i][1] = transformPoints([pos.slice()])[1]
